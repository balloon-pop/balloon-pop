...
func _on_bird_screen_exited() -> void:
	var is_bird_below_player = position.y > PlayerManager.position.y

	# bird가 화면을 빠져나갈 때 플레이어보다 아래에 있었을 때만 제거
	if is_bird_below_player:
		queue_free()

func _ready() -> void:
	change_state(State.IDLE)
	hitBox.body_entered.connect(_on_hitBox_body_entered)
	visibleNotifier.screen_exited.connect(_on_bird_screen_exited)
